...
namespace BUS
{
    public class sanPhamBUS
    {
        ...
        public BindingList<sanPhamDTO> timKiemBaseSP(string tim, int index)
        {
            ...
            BindingList<loaiDTO> dsl = new loaiSanPhamBUS().LayDanhSach();
            foreach (sanPhamDTO ct in ds)
            {
                switch (index)
                {
                    ...
                    case 2:
                        {
                            loaiDTO nl = dsl.FirstOrDefault(x => x.MaLoai == ct.MaLoai);
                            string tenNL = nl != null ? nl.TenLoai : "";
